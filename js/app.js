	...
	var Main = function () {
	  ...
	  _createClass(Main, [{
	    ...
	    value: function main(e) {
	      ...
	      var texturePlayer, textureSpAtk, textureSnow4, textureSnow8, textureSnow16, textureSnow24, textureSnow32, textureSnow40, textureSnow48, textureSnow56, textureSnow64, textureBit, textureBitDisabled;
	      ...
	      function loadObjects() {
	        ...
	        loader.add('./img/playerimg.png').add('./img/umbrellaimg.png').add('./img/snowimg4.png').add('./img/snowimg8.png').add('./img/snowimg16.png').add('./img/snowimg24.png').add('./img/snowimg32.png').add('./img/snowimg40.png').add('./img/snowimg48.png').add('./img/snowimg56.png').add('./img/snowimg64.png')
	        // .add('./img/bg1.png')
	        .add('./img/bg2.png').add('./img/bit.png').add('./img/bitd.png').load(pixiLoaderCallBack);
	          ...
	          case 6:
	            ...
	            if (currentOffsettedFrame > 900 && frameCnt % 10 === 1) {
	              ...
	              for (var _i3 = 0; _i3 < 8; _i3++) {
	                addBullet(71, currentOffsettedFrame, { defX: myDefX, defY: myDefY, radius: 80, offsetDegree: randomDegree + 45 * _i3 });
	              }
	            }
	            ...
	            if (currentOffsettedFrame > 2700) {
	              ...
	              if ((frameCnt + 5) % 4 === 1) {
	                ...
	                for (var _i4 = 0; _i4 < 8; _i4++) {
	                  addBullet(71, currentOffsettedFrame, { defX: myDefX, defY: myDefY, radius: 80, offsetDegree: randomDegree + 45 * _i4 });
	                }
	              }
	            }
	            break;
	      ...
	      function spHitCheck() {
	        if (spatk.isUsed !== true) {
	          ...
	          for (var i = 0, j = bullets.length; i < j; i++) {
	            ...
	            myBltX = bullets[i].x;
	            myBltY = bullets[i].y;
	            if (myAbs(myBltX - mySPX) < myHitRadius && myAbs(myBltY - mySPY) < myHitRadius) {
	              //先に軽く矩形で判定し、通った場合のみ半径で判定する
	              // myTrace('check');
	              if (myPow2(myBltX - mySPX) + myPow2(myBltY - mySPY) <= myHitRadius * myHitRadius) {
	                //平方根はおそそうだから使わず二乗のまま比較する
	      ...
	      function hitCheck(num) {
	        ...
	        if (myAbs(bullets[num].x - myPlayerX) < myHitRadius && myAbs(bullets[num].y - myPlayerY) < myHitRadius) {
	          // 矩形で判定するほうが速いので
	          player.isHit = true;
	      ...
	      function callBG() {
	        ...
	        // shadowControls
	        var sampleSquare = function sampleSquare() {
	          this.x = 0;
	          this.y = 600;
	          this.z = 0;
